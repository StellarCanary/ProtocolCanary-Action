...
| `passed` | Number of checks that passed. When `status` is `execution-failed`, this is forced to `"0"` (not left unset). |
| `warnings` | Number of checks that produced a warning. When `status` is `execution-failed`, this is forced to `"0"`. |
| `failures` | Number of checks that failed a compatibility assertion. When `status` is `execution-failed`, this is forced to `"0"`. |
| `errors` | Number of checks that could not complete due to an execution error. When `status` is `execution-failed`, this is forced to `"0"`. |
